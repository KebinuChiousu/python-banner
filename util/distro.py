...
class LinuxDistribution(object):
    ...
    def id(self):
        ...
        def normalize(distro_id, table):
            """ standardize formatting for processing. """
    ...
    def name(self, pretty=False):
        ...
        """
        _name = self.os_release_attr('name') \
            or self.lsb_release_attr('distributor_id') \
            or self.distro_release_attr('name') \
            or self.uname_attr('name')
        if pretty:
            _name = self.os_release_attr('pretty_name') \
                or self.lsb_release_attr('description')
            if not _name:
                _name = self.distro_release_attr('name') or self.uname_attr('name')
                _version = self.version(pretty=True)
                if _version:
    ...
    def version(self, pretty=False, best=False):
        ...
        if best:
            ...
            for v in versions:
                if v.count(".") > _version.count(".") or _version == '':
                    _version = v
        else:
            for v in versions:
                if v != '':
                    _version = v
                    break
        if pretty and _version and self.codename():
            _version = u'{0} ({1})'.format(_version, self.codename())
        return _version
    ...
    def version_parts(self, best=False):
        ...
        if version_str:
            ...
            if matches:
                major, minor, _build_number = matches.groups()
                return major, minor or '', _build_number or ''
        return '', '', ''
        ...
        for token in tokens:
            ...
            if '=' in token:
                ...
                if k == 'VERSION':
                    ...
                    _codename = re.search(r'(\(\D+\))|,(\s+)?\D+', v)
                    if _codename:
                        _codename = _codename.group()
                        _codename = _codename.strip('()')
                        _codename = _codename.strip(',')
                        _codename = _codename.strip()
                        # codename appears within paranthese.
                        props['codename'] = _codename
                    else:
                        props['codename'] = ''
            else:
    ...
    def _parse_uname_content(lines):
        ...
        if match:
            ...
            if _name == 'Linux':
                return {}
            props['id'] = _name.lower()
            props['name'] = _name
            props['release'] = _version
        return props
